...
#ifdef CONFIG_SPECTRA_CAMERA_HBSP_DEBUG
		memcpy(s_ctrl->sensordata->slave_info.sensor_name,
			sensor_probe_info_v2->sensor_name,sizeof(sensor_probe_info_v2->sensor_name));
		CAM_ERR(CAM_SENSOR,"sensor name: %s", s_ctrl->sensordata->slave_info.sensor_name);
#endif
...
#ifdef CONFIG_SPECTRA_CAMERA_HBSP_DEBUG
	if(s_ctrl->sensordata->hbsp_cam_pos == 0){
		productinfo_register(PRODUCTINFO_MAIN_CAMERA_ID, s_ctrl->sensordata->slave_info.sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "main camera : %s",s_ctrl->sensordata->slave_info.sensor_name);
	}else if(s_ctrl->sensordata->hbsp_cam_pos ==1){
		productinfo_register(PRODUCTINFO_FRONT_CAMERA_ID, s_ctrl->sensordata->slave_info.sensor_name, NULL);
		CAM_ERR(CAM_SENSOR, "front camera : %s",s_ctrl->sensordata->slave_info.sensor_name);
	}else{
		CAM_ERR(CAM_SENSOR, "camera error productinfo");
	}
 #endif /* CONFIG_HBSP_CAMERA_DEBUG */
