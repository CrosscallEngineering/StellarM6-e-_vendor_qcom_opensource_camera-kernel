...
#ifdef CONFIG_SPECTRA_CAMERA_HBSP_DEBUG
...
#define LED_5 5
#define LED_6 6
static int flash_write_addr8_data8(struct cam_flash_ctrl *fctrl,
       uint32_t addr, uint8_t addr_type, uint8_t data, uint8_t data_type)
{
       ...
       if (fctrl == NULL) {
               CAM_ERR(CAM_FLASH, "Invalid Args fctrl: %pK, data: %pK",
                       fctrl, data);
               return -EINVAL;
       }

       fctrl->io_master_info.cci_client->i2c_freq_mode = I2C_FAST_MODE;
       //fctrl->io_master_info.cci_client->cci_i2c_master = CCI_MASTER_1;
       ...
       i2c_reg_setting.reg_setting =
           ...
           kzalloc(sizeof(struct cam_sensor_i2c_reg_array) , GFP_KERNEL);
       ...
       ret = camera_io_dev_write(&(fctrl->io_master_info), &i2c_reg_setting);
       if (ret < 0) {
               CAM_ERR(CAM_FLASH, "Failed to random write I2C settings: %d", ret);
       } else {
       		CAM_DBG(CAM_FLASH, "write I2C settings succeed: %d", ret);
       }
       kfree(i2c_reg_setting.reg_setting);
       return ret;
}

void hbsp_flash_brightness_set(struct led_classdev *led_cdev,
    enum led_brightness led_value)
{
       int rc = 0;
       int value = led_value;
       struct cam_flash_ctrl *fctrl = container_of(led_cdev, struct cam_flash_ctrl, led_dev);
    
       CAM_ERR(CAM_FLASH, " E.\n");
       if (fctrl == NULL) {
               CAM_ERR(CAM_FLASH, " fctrl == NULL. Fail!\n");
               return;
       }
	if (fctrl->is_flash_used != 0){
		CAM_ERR(CAM_FLASH,"camera is on, don't use flash node\n");
		return;
	}
	CAM_DBG(CAM_FLASH,"fctrl->cci_i2c_master:%d, fctrl->io_master_info.cci_client->cci_i2c_master:%d\n",
		fctrl->cci_i2c_master, fctrl->io_master_info.cci_client->cci_i2c_master);
	fctrl->io_master_info.cci_client->cci_i2c_master = fctrl->cci_i2c_master;

	if ((value == LED_OFF) && fctrl->flash_state == CAM_FLASH_STATE_START) {
	      //off setting
	      rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE, 
		                 0x80,CAMERA_SENSOR_I2C_TYPE_BYTE);
		if (rc){
	      		CAM_ERR(CAM_FLASH,"flash/torch off failed : %d", rc);
		}
		//power down
		if (fctrl->func_tbl.power_ops) {
		        fctrl->func_tbl.power_ops(fctrl, false);
			//gpio_set_value_cansleep(fctrl->power_info.gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO1], 0);
		}
	...
	if (value != LED_OFF ) {
		if (fctrl->flash_state == CAM_FLASH_STATE_INIT){
		   	//power up & init i2c
	               if (fctrl->func_tbl.power_ops) {
	                       fctrl->func_tbl.power_ops(fctrl, true);
		       }
		       //gpio_set_value_cansleep(fctrl->power_info.gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO1], 1);
		       mdelay(100);
		       //slave address
		       fctrl->io_master_info.cci_client->sid = 0xc6 >> 1;
		       //init setting
	               rc = flash_write_addr8_data8(fctrl, 0x07,CAMERA_SENSOR_I2C_TYPE_BYTE, 
				               0x89,CAMERA_SENSOR_I2C_TYPE_BYTE);
		       mdelay(1);
	               rc = flash_write_addr8_data8(fctrl, 0x08,CAMERA_SENSOR_I2C_TYPE_BYTE, 
	                           0x1A,CAMERA_SENSOR_I2C_TYPE_BYTE);
		       mdelay(1);
	                if (rc)
	                      CAM_ERR(CAM_FLASH,"flash/torch init failed : %d", rc);
       		}
	  
		switch (value) {
		case LED_1: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 1:15x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x07,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x07,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);

			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 1 failed : %d", rc);
			break;
		}
		case LED_2: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 2:45x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x17,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x17,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);
			...
			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 2 failed : %d", rc);
			break;
		}
		case LED_3: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 3:75x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x26,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x26,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);
			...
			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 3 failed : %d", rc);
			break;
		}
		case LED_4: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 4:105x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x35,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x35,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);
			...
			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 4 failed : %d", rc);
			break;
		}
		case LED_5: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 5:140x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x47,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x47,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);
			...
			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 5 failed : %d", rc);
			break;
		}
		case LED_6: {
			//CAM_ERR(CAM_FLASH,"LED LEVEL 6:200x2\n");
			rc = flash_write_addr8_data8(fctrl, 0x05,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x65,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x06,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x65,CAMERA_SENSOR_I2C_TYPE_BYTE);
			rc = flash_write_addr8_data8(fctrl, 0x01,CAMERA_SENSOR_I2C_TYPE_BYTE,
				0x8B,CAMERA_SENSOR_I2C_TYPE_BYTE);
			...
			if (rc)
				CAM_ERR(CAM_FLASH,"TORCH ON 6 burst failed : %d", rc);
			break;
		}
		default:
			CAM_ERR(CAM_FLASH, " invaild cmd = %d X.\n", value);
		}
