...
#ifdef CONFIG_SPECTRA_CAMERA_HBSP_DEBUG
static int32_t hbsp_torch_create_classdev(struct platform_device *pdev,
				void *data)
{
	int32_t rc = 0;
	struct cam_flash_ctrl *fctrl =
		(struct cam_flash_ctrl *)data;

	if (!fctrl) {
		pr_err("Invalid fctrl\n");
		return -EINVAL;
	}
	fctrl->led_dev.brightness_set = hbsp_flash_brightness_set;
	fctrl->led_dev.brightness = LED_OFF;
	/*note:index = 0 for rear flash,index = 1 for front flash*/
	pr_err("fctrl->soc_info.index = %d\n", fctrl->soc_info.index);
	if (fctrl->soc_info.index == 0) {
		fctrl->led_dev.name = "flashlight";
		...
		//hbsp_flash_brightness_set(&fctrl->led_dev, LED_OFF);
	} else if (fctrl->soc_info.index == 1) {
		fctrl->led_dev.name = "front_flashlight";
		rc = led_classdev_register(&pdev->dev,
				&fctrl->led_dev);
		pr_err("front falsh");
		if (rc) {
...
#ifdef CONFIG_SPECTRA_CAMERA_HBSP_DEBUG
	rc = hbsp_torch_create_classdev(pdev, fctrl);
#endif
